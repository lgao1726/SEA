 ...
 */
void MailboxWriteGraphic(uint32 message, uint32 mailbox)
{
  uint32 status;
 ...
 */
uint32 MailboxReadGraphic(uint32 mailbox)
{
  uint32 status;
...
int FramebufferInitialize() {
  ...
  MailboxWriteGraphic((uint32)(mb+0x40000000), 8); // On écrit le message dans la mailbox
  
  if(((retval = MailboxReadGraphic(8)) == 0) || (mb[1] != 0x80000000)){ // On vérifie que le message est passé
    return 0;
  }
  ...
  MailboxWriteGraphic((uint32)(mb+0x40000000), 8); // On écrit dans la mailbox
  
  if(((retval = MailboxReadGraphic(8)) == 0) || (mb[1] != 0x80000000)){ // On vérifie que le message a bien été passé
    return 0;
  }
  ...
  MailboxWriteGraphic((uint32)(mb+0x40000000), 8);
  
  if(((retval = MailboxReadGraphic(8)) == 0) || (mb[1] != 0x80000000)){
    return 0;
  }
...
void drawTest(){
	...
	while(i<testSize){
		if(column==testWidth-1){
			row++;
			column = 0;
		}

		put_pixel_RGB24(column,row,pic[i],pic[i+1],pic[i+2]);
		column++;
		i += 4;
	}
